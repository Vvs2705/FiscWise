...
async def _send_otp_email(email: str, code: str) -> None:
    """Send the 2FA OTP code via e-mail. Logs the code ONLY in DEBUG mode."""
    sent = await send_email(
        to=email,
        subject="Seu código FiscWise",
        html=(
            "<p>Seu código de verificação FiscWise é:</p>"
            f"<p style=\"font-size:24px;font-weight:bold;letter-spacing:4px\">{code}</p>"
            f"<p>Ele expira em {_EMAIL_OTP_TTL_MINUTES} minutos. "
            "Se você não solicitou este código, ignore este e-mail.</p>"
        ),
    )
    if settings.DEBUG:
        # Nunca logar o OTP em produção — apenas em DEBUG para desenvolvimento local
        logger.info("EMAIL OTP for %s: %s", email, code)
    elif not sent:
        logger.warning("Falha ao enviar OTP 2FA por e-mail para %s", email)
...
async def login(
    ...
    if getattr(user, "two_factor_enabled", False):
        ...
        if method == "email":
            ...
            await _send_otp_email(user.email, otp_code)
...
@router.post("/login/resend-2fa", response_model=Resend2FAResponse, summary="Resend Email OTP")
async def resend_2fa_code(
    body: Resend2FARequest,
    db: AsyncSession = Depends(get_db),
):
    """
    Re-generate and re-send the email OTP for users whose 2FA method is 'email'.
    Only valid for the email OTP method; TOTP codes are generated by authenticator apps.
    Rate limiting is handled by the existing auth rate limiter at the route level.
    """
    ...
    if method != "email":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Reenvio de código não disponível para autenticação por app TOTP.",
        )
    ...
    if existing_user:
        ...
        if getattr(existing_user, "two_factor_enabled", False):
            ...
            if method == "email":
                ...
                await _send_otp_email(existing_user.email, otp_code)
...
# Stateless: o token de reset é um JWT (purpose=pwd_reset, TTL 30min) assinado
# com o mesmo JWT_SECRET_KEY — nenhuma migration necessária.

_FRONTEND_URL = os.getenv("FRONTEND_URL", "https://www.fiscwise.com.br")
...
class ResetPasswordRequest(BaseModel):
    token: str
    # Mesma regra de força do registro (min 8); max 72 = limite do bcrypt
...
def _verify_reset_token(token: str) -> Optional[str]:
    """Return user_id if the token is a valid pwd_reset JWT, else None."""
    try:
...
async def forgot_password(
    ...
    """
    Solicita redefinição de senha. SEMPRE responde 202 com a mesma mensagem,
    exista ou não o e-mail (evita user enumeration).

    Rate limited: max 5 pedidos / 15 min por IP e por e-mail.
    """
    client_ip = get_client_ip(request)
    auth_rate_limiter.check_request_rate("pwd_reset_ip", client_ip, 5, 15)
    auth_rate_limiter.check_request_rate("pwd_reset_email", body.email.lower(), 5, 15)
    ...
    if user and user.is_active:
        ...
        sent = await send_email(
            to=user.email,
            subject="Redefinição de senha — FiscWise",
            html=(
                "<p>Recebemos um pedido para redefinir a senha da sua conta FiscWise.</p>"
                f"<p><a href=\"{reset_link}\">Clique aqui para criar uma nova senha</a> "
                f"(válido por {_RESET_TOKEN_TTL_MINUTES} minutos).</p>"
                "<p>Se você não solicitou esta redefinição, ignore este e-mail — "
                "sua senha permanece inalterada.</p>"
            ),
        )
        if not sent:
            logger.warning("E-mail de reset de senha não enviado para %s", user.email)
...
async def reset_password(
    ...
    Token inválido/expirado → 400 com mensagem genérica.
    """
